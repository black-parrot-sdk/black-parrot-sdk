...
bsg_log_init() {
    ...
    if [ -f "${_rptfile}.0" ]; then
        suffix=.$(stat ${_rptfile}.* --printf "%i\n" | wc -l)
    else
        suffix=.0
    fi
    ...
    if [ -f "${_outfile}.0" ]; then
        suffix=.$(stat ${_outfile}.* --printf "%i\n" | wc -l)
    else
        suffix=.0
    fi
    _BSG_OUT_FILE="${_outfile}${suffix}"

    printf "logfile: ${_BSG_LOG_FILE} with log level ${_BSG_LOG_LEVEL}\n"
    printf "loglevel: ${_BSG_LOG_LEVEL}\n"
